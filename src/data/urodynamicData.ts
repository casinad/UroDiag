...
function determinerDiagnostic(
  ...
  const compliance = data.cystometrie.compliance;
  const rpm = data.residuPostMictionnel;
  ...
  // Obstruction sévère avec vessie de lutte (cas 11)
  if (indexObstruction > 40) {
    if (dyssynergie) {
      return 'Dyssynergie vésico-sphinctérienne avec obstruction fonctionnelle';
    } else if (data.sexe === 'M' && data.antecedents.includes('HBP')) {
      // Cas spécifique de vessie de lutte
      if (pdetQmax > 80 && compliance < 10 && rpm > 100) {
        return 'Obstruction prostatique sévère avec vessie de lutte';
      } else {
...
function genererRecommandations(data: PatientData, diagnostic: string): Array<{ label: string; tooltip: string }> {
  ...
  if (diagnostic.includes('Obstruction prostatique')) {
    ...
    // Recommandation spécifique pour vessie de lutte
    if (diagnostic.includes('vessie de lutte')) {
      recommandations.push({
        label: 'Évaluer la taille prostatique avant d\'envisager un inhibiteur de la 5-alpha-réductase',
        tooltip: 'Les inhibiteurs de la 5-alpha-réductase (finastéride, dutastéride) sont indiqués si le volume prostatique >30-40 ml. Ils réduisent la taille de la prostate de 20-30% en 6-12 mois et diminuent le risque de progression et de rétention aiguë.'
      });
      
      recommandations.push({
        label: 'Envisager une chirurgie de désobstruction si traitement médical inefficace ou rétention récidivante',
        tooltip: 'La résection trans-urétrale de prostate (RTUP) est le traitement définitif en cas d\'obstruction sévère avec vessie de lutte. Indiquée si échec médical, rétention récidivante, ou altération du haut appareil urinaire. Amélioration du Qmax de 100-200% dans 85-90% des cas.'
      });
    } else if (data.etudePressionDebit.pressionDetrusorQmax > 60) {
      recommandations.push({
...
function genererExamensComplementaires(data: PatientData, diagnostic: string): Array<{ label: string; tooltip: string }> {
  ...
  // Examen spécifique pour vessie de lutte
  if (diagnostic.includes('vessie de lutte') || data.residuPostMictionnel > 100) {
    examens.push({
...
function genererTraitements(data: PatientData, diagnostic: string): Array<{ label: string; tooltip: string }> {
  ...
  if (diagnostic.includes('Obstruction prostatique')) {
    ...
    // Traitement spécifique pour vessie de lutte
    if (diagnostic.includes('vessie de lutte') || data.profilPression.pressionUretrale > 80) {
      traitements.push({
      ...
      traitements.push({
        label: 'RTUP (résection transurétrale de la prostate) en cas d\'échec médical',
        tooltip: 'La RTUP est le traitement chirurgical de référence pour l\'obstruction prostatique sévère. Indiquée en cas d\'échec du traitement médical, de rétention récidivante, ou de complications (infections, altération rénale). Taux de succès >90% avec amélioration durable des symptômes.'
      });
    }
  }
...
function genererPieges(data: PatientData, diagnostic: string): Array<{ label: string; tooltip: string }> {
  ...
  // Pièges spécifiques pour vessie de lutte
  if (diagnostic.includes('vessie de lutte') || data.cystometrie.compliance < 10) {
    pieges.push({
      label: 'Ne pas ignorer une composante de vessie de lutte en cas de Pdet élevé',
      tooltip: 'Une pression détrusorienne élevée (>80 cmH2O) face à une obstruction chronique peut entraîner une hypertrophie détrusorienne et une altération de la compliance vésicale. Cette "vessie de lutte" expose à un risque de décompensation et d\'atteinte du haut appareil urinaire.'
    });
    
    pieges.push({
      label: 'Corréler obstruction clinique et tolérance vésicale : un RPM élevé n\'est pas toujours symptomatique',
      tooltip: 'Un résidu post-mictionnel élevé peut être bien toléré cliniquement au début de l\'évolution, mais expose à un risque de décompensation tardive (rétention aiguë, infections récidivantes, insuffisance rénale). La surveillance régulière est indispensable même en l\'absence de symptômes.'
    });
  }
